		...
		*/
		this.percolateDown = function ( index ) {
			...
			while ( 2 * pos < this.sizeNow ) { 
				...
				if ( 2 * pos+1 < this.sizeNow && score[comparitor] > score[rightChild] ) {
		...
		this.dump_r = function (  level , depth) {
			if ( level == 0 ) {
				...
				return outstring + score[this.theHeap[1]];
			}
			...
			for ( var j = 0; j < x && x+j < this.sizeNow; j += 1 ) {
					outstring = outstring.concat(score[this.theHeap[x+j]] + ' ');
			}
			return outstring;
		};
	}
